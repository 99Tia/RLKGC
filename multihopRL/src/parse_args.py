...
# CATS-style Explanation Bundles
parser.add_argument('--cats_same_entity_paths', action='store_true', default=False,
                    help='Collect reasoning paths that all end at the same candidate entity')
parser.add_argument('--cats_target_selector', type=str, default='beam', choices=['beam', 'kge'],
                    help="How to choose the single candidate entity e*: 'beam' = top-1 from beam search, 'kge' = top-1 from fact scorer")
parser.add_argument('--cats_depth', type=int, default=3,
                    help='Maximum BFS depth (path length) for collecting reasoning paths')
parser.add_argument('--cats_branch_cap', type=int, default=10,
                    help='Maximum number of outgoing edges to expand per node during BFS')
parser.add_argument('--cats_paths_per_entity', type=int, default=5,
                    help='Number of reasoning paths to keep per query/entity')
parser.add_argument('--cats_output_jsonl', type=str, default='',
                    help='Optional path to save exported bundles in JSONL format (empty = no file output)')
