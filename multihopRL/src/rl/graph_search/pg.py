...
class PolicyGradient(LFramework):
    ...
    def predict(self, mini_batch, verbose=False):
        ...
        # === NEW: build CATS-style bundles (same-tail reasoning paths) ===
        cats_bundles = None
        if getattr(kg.args, 'cats_same_entity_paths', False):
            from src.cats_utils import bounded_bfs_to_target, score_path_logprob, format_path_for_export
            cats_bundles = []
            B = len(e1)
            for i in range(B):
                # (1) choose one candidate tail e*
                if kg.args.cats_target_selector == 'beam':
                    e_star = int(pred_e2s[i, 0])  # beam top-1
                else:  # 'kge' → use fact scorer (TripleE or its variants)
                    if not hasattr(self, 'fn'):
                        # plain PG (no fact network) → fallback to beam
                        e_star = int(pred_e2s[i, 0])
                    else:
                        with torch.no_grad():
                           
                           if hasattr(self, 'tertiary_kg'):   # TripleE ensemble
                               scores = self.fn.forward_fact(
                                   e1[i:i+1], r[i:i+1], self.fn_kg, 
                                   [self.secondary_kg, self.tertiary_kg]
                                )
                           elif hasattr(self, 'secondary_kg'):
                                scores = self.fn.forward_fact(
                                    e1[i:i+1], r[i:i+1], self.fn_kg, 
                                    [self.secondary_kg]
                                )
                        ...
                        e_star = int(torch.argmax(scores[0]))
                

                # (2) collect paths to the SAME tail with bounded, policy-guided BFS
                paths = bounded_bfs_to_target(
                    pn=self.mdl, kg=self.kg,
                    e_s=int(e1[i]), q=int(r[i]), e_star=e_star,
                    max_depth=kg.args.cats_depth, branch_cap=kg.args.cats_branch_cap
                    )

                # (3) score and keep top-M
                if not paths:
                    topM = []
                else:
                    scored = [(score_path_logprob(self.mdl, self.kg, int(e1[i]), int(r[i]), p), p) for p in paths]
                    scored.sort(key=lambda x: x[0], reverse=True)
                    topM = scored[:kg.args.cats_paths_per_entity]

                # (4) format a clean bundle for this query
                bundle = {

                    'query':  {'e1_id': int(e1[i]), 'r_id': int(r[i]),
                               'e1': self.kg.id2entity[int(e1[i])],
                               'r':  self.kg.id2relation[int(r[i])]},
                    'candidate': {'e2_id': e_star, 'e2': self.kg.id2entity[e_star]},
                    'paths': [format_path_for_export(p, self.kg, logprob=lp) for (lp, p) in topM]
                    }
                cats_bundles.append(bundle)
        ...
        self.cats_bundles = cats_bundles
        return pred_scores
