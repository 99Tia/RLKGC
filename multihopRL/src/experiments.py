...
def inference(lf):
    ...
    if hasattr(lf, "cats_bundles") and lf.cats_bundles and args.cats_output_jsonl:
        out_dir = os.path.dirname(args.cats_output_jsonl)
        if out_dir:
            os.makedirs(out_dir, exist_ok=True)
        with open(args.cats_output_jsonl, "w", encoding="utf-8") as f:
            for bundle in lf.cats_bundles:
                f.write(json.dumps(bundle, ensure_ascii=False) + "\n")
        print(f"[CATS] wrote {len(lf.cats_bundles)} bundles to {args.cats_output_jsonl}")
    return eval_metrics
