...
def build_instructions(dataset, train_size, subgraph_type, neg_num, version):
    ...
    use_embedder = (subgraph_type in ["combine", "neighbor-only"])
    data_manager = DataManager(dataset=dataset, setting=setting, train_size=train_size, use_sentence_embedder=False)
    ...
    for pos_triple in tqdm(data_manager.path_set, desc=f"Processing {dataset} - setting: {setting} - Train_size: {train_size}"):
        ...
        if subgraph_type == "combine":
            ...
            pos_close_paths = data_manager.close_path_finder(pos_triple)
            pos_reasoning_paths = "\n".join(
        ...
        for neg_triple in neg_samples:
            ...
            if subgraph_type == "combine":
                ...
                neg_close_paths = data_manager.close_path_finder(neg_triple)
                neg_reasoning_paths = "\n".join(
                    " -> ".join(data_manager.triple_to_sentence(triple) for triple in path)
...
def build_vanilla_instructions(dataset, train_size, neg_num, version, subgraph_type="combine"):
    setting = "transductive" # 指令构建默认是transductive，用训练集
    
    # data_manager = DataManager(dataset=dataset, setting=setting, train_size=train_size)
    use_embedder = (subgraph_type in ["combine", "neighbor-only"])
    data_manager = DataManager(dataset=dataset, setting=setting, train_size=train_size,
                           use_sentence_embedder=use_embedder)
...
def main():
    ...
    parser.add_argument("--dataset", type=str, choices=["FB15k-237-subset", "NELL-995-subset", "WN18RR-subset", "UMLS", "Kinship"], default="FB15k-237-subset")
    ...
    if args.prompt_type == "CATS":
        ...
        build_vanilla_instructions(args.dataset, args.train_size, args.neg_num, args.version, args.subgraph_type)
