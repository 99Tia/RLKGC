...
LLM_PATH = "/home/ib5539/models"

class DataManager:
    def __init__(self, dataset="FB15k-237-subset", setting="inductive", train_size="full", model_name="Qwen2-7B-Instruct", llm_type="sft", use_sentence_embedder=False):
        self.dataset = dataset
        self.model_name = model_name
        # self.dataset_name = dataset.split("-")[0]
        self.dataset_tag = dataset
        ...
        # else:
        #     if llm_type == "sft":
        #         self.model_path = f"{LLM_PATH}/{model_name}-{self.dataset_name}-{train_size}"
        #     else:
        #         self.model_path = f"{LLM_PATH}/{model_name}"

        # if os.path.isabs(model_name) and os.path.exists(model_name):
        #     self.model_path = model_name
        # else:
        #     if llm_type == "sft":
        #         self.model_path = f"{LLM_PATH}/{model_name}-{self.dataset_tag}-{train_size}"
        #     else:
        #         self.model_path = f"{LLM_PATH}/{model_name}"
        ...
        if os.path.isabs(model_name) or model_name.startswith("./") or model_name.startswith("../"):
            # Treat as local path exactly as given
            self.model_path = os.path.abspath(model_name)
        else:
            # Treat as a name under LLM_PATH (or a HF hub id if it’s not local)
            if llm_type == "sft":
                self.model_path = f"{LLM_PATH}/{model_name}-{self.dataset_tag}-{train_size}"
            else:
                self.model_path = f"{LLM_PATH}/{model_name}"
        # --- end resolve model path ---
        ...
        self.max_reason_paths = 7                                    # Path Reasoning阶段最多使用6个path，其中neighbor_triples和close_paths都最多六个
        ...
        #         print(f"[WARN] Could not load SentenceTransformer: {e}. Falling back to simple neighbor selection.")
        #         self.embedding_model = None
        # if use_sentence_embedder:
        #     try:
        #         from sentence_transformers import SentenceTransformer
        ...
        # self.few_shot_k = 3  # num examples
        # self.support_examples = self.train_set[:self.few_shot_k]
    ...
    #     examples = []
    #     for h, r, t in self.support_examples:
    #         text = self.triple_to_sentence((h, r, t))
    #         examples.append(f"Example: {text}\nAnswer: True")
    #     return "\n".join(examples) + "\n"
    ...
    def _ensure_embedder(self):
        if self.embedding_model is None:
            try:
                ...
                print(f"[WARN] Could not load BGE on GPU ({e}). Falling back to CPU.")
                self.embedding_model = SentenceTransformer('BAAI/bge-small-en-v1.5', device='cpu')
    ...
    def close_path_finder(self, triple):
        ...
        key = f"{head}-{tail}"
        # fall back to reverse key if present (some exporters may store reversed)
        close_paths = self.close_path_dict.get(key, self.close_path_dict.get(f"{tail}-{head}", []))
    ...
    def close_path_finder_no_degree(self, triple):
        head, relation, tail = triple
        head_tail = f"{head}-{tail}"
        close_paths = self.close_path_dict.get(head_tail, self.close_path_dict.get(f"{tail}-{head}", []))
        return close_paths[:self.max_reason_paths] if close_paths else []
    ...
    def triple_to_sentence(self, triple):
        ...
        if self.dataset == "FB15k-237-subset":
            ...
            return f"('{self.entity2text[head]}' {self.relation2text[relation]} '{self.entity2text[tail]}')"
        elif self.dataset in ["UMLS", "UMLS-inductive"]:
            return f"('{self.entity2text[head]}' {self.relation2text[relation]} '{self.entity2text[tail]}')"
        elif self.dataset in ["Kinship", "Kinship-inductive"]:
            h = self.entity2text.get(head, str(head))
            r = self.relation2text.get(relation, str(relation))
            t = self.entity2text.get(tail, str(tail))
            return f"('{h}' {r} '{t}')"
    ...
    def neighbor_triple_finder(self, triple):
        ...
        top_head_sentences = head_sentences[:each_count]
        top_tail_sentences = tail_sentences[:each_count]

        # if self.embedding_model is not None:
        #     if len(head_sentences) > each_count:
        #         head_embeddings = self.embedding_model.encode(
        #         [triple_sentence] + head_sentences, normalize_embeddings=True
        #         )
        ...
        #         tail_embeddings = self.embedding_model.encode(
        #         [triple_sentence] + tail_sentences, normalize_embeddings=True
        #         )
        ...
        if self.embedding_model is not None:
            if len(head_sentences) > each_count:
                head_embeddings = self.embedding_model.encode([triple_sentence] + head_sentences, normalize_embeddings=True)
                sims = head_embeddings[0] @ head_embeddings[1:].T
                top_idx = np.argsort(-sims)[:each_count]
                top_head_sentences = [head_sentences[i] for i in top_idx]
            if len(tail_sentences) > each_count:
                tail_embeddings = self.embedding_model.encode([triple_sentence] + tail_sentences, normalize_embeddings=True)
                sims = tail_embeddings[0] @ tail_embeddings[1:].T
                top_idx = np.argsort(-sims)[:each_count]
                top_tail_sentences = [tail_sentences[i] for i in top_idx]
