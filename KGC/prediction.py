...
def cal_Y_prob(model:AutoModelForCausalLM, tokenizer:AutoTokenizer, generation_config, prompt_list):
    ...
    inputs = tokenizer(
        ...
        padding=True,
        truncation=True,
        max_length=getattr(tokenizer, "model_max_length", 4096) or 4096,
    ).to("cuda")
    ...
    Y_id = _first_id("Y") or _first_id(" Y")
    N_id = _first_id("N") or _first_id(" N")
...
def main():
    ...
    parser.add_argument("--dataset", type=str, choices=["FB15k-237-subset", "NELL-995-subset", "WN18RR-subset", "UMLS"], default="UMLS", help="Name of the dataset")
    ...
    # parser.add_argument("--model_name", type=str, choices=["Qwen2-7B-Instruct", "Meta-Llama-3-8B-Instruct", "Qwen2-1.5B-Instruct", "/home/ib5539/models/Qwen2-7B-SFT-UMLS"], default="Qwen2-7B-Instruct")
    parser.add_argument("--model_name", type=str, default="Qwen2-7B-Instruct", help="Model name or local path to fine-tuned model")
    ...
    # things i did for llama3 WN18RR
    # model = AutoModelForCausalLM.from_pretrained(
    #     data_manager.model_path,
    #     torch_dtype="auto",
    #     device_map="auto",
    #     local_files_only=True
    # )
    tokenizer = AutoTokenizer.from_pretrained(data_manager.model_path, local_files_only=True)

    # LLaMA3 and other models missing pad token
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
    tokenizer.padding_side = "left"
    # added for llama3 WN18RR
    # model = AutoModelForCausalLM.from_pretrained(data_manager.model_path, torch_dtype="auto", device_map="auto")
    model = AutoModelForCausalLM.from_pretrained(
        data_manager.model_path,
        torch_dtype="auto",
    ...
    # Ensure a concrete max length for truncation (use model config if present, for llama3 WN18RR)
    model_max_len = getattr(model.config, "max_position_embeddings", None)
    if model_max_len is None or model_max_len > 8192:
        model_max_len = 4096  # safe default
